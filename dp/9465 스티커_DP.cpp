...
int main() {
	...
	while (t--) {
		...
		for (int i = 0; i < n; i++) {
			dp[i + 1][0] = max(max(dp[i + 1][1] + sticker[1][i], dp[i + 1][2] + sticker[0][i]), dp[i + 1][0]); 
			dp[i + 1][1] = max(dp[i + 1][2] + sticker[0][i], dp[i + 1][0]);
			dp[i + 1][2] = max(dp[i + 1][1] + sticker[1][i], dp[i + 1][0]);
		}
